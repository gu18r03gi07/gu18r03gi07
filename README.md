# 👨‍💻 Gustavo Rodrigues Giamarco

### 🚀 Junior Software Developer | Full Stack Developer

<div align="center">

![Status](https://img.shields.io/badge/Status-Em%20Constante%20Evolução-brightgreen?style=flat-square)
![Learning](https://img.shields.io/badge/Learning-Java%20%26%20Full%20Stack-blue?style=flat-square)
![Open for Collaboration](https://img.shields.io/badge/Open%20for-Collaboration-success?style=flat-square)

</div>

Olá! Eu sou **Gustavo Rodrigues Giamarco**, estudante de Engenharia de Software e desenvolvedor em formação, apaixonado por tecnologia e criação de soluções digitais.

Atualmente estou focado em **Java, desenvolvimento Web, APIs, bancos de dados, Angular, Inteligência Artificial e automação**.

---

## 🧑‍💻 Sobre mim

🎓 Estudante de Engenharia de Software  
💻 Desenvolvedor de Software em formação  
☕ Foco principal em Java  
🌐 Desenvolvimento Full Stack  
🗄️ Banco de dados e SQL  
🤖 Inteligência Artificial e automação  
🔧 Git e GitHub  
📚 Aprendizado contínuo  

**Meu objetivo:** Transformar ideias e problemas em **soluções de software eficientes, organizadas e escaláveis**.

---

## 🛠️ Tecnologias

### 💻 Linguagens

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### 🌐 Front-end

![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### ⚙️ Backend

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-02569B?style=for-the-badge&logo=postman&logoColor=white)

### 🗄️ Banco de Dados

![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![MySQL Workbench](https://img.shields.io/badge/MySQL%20Workbench-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

### 🔧 Ferramentas & IDEs

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ%20IDEA-000000?style=for-the-badge&logo=intellijidea&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white)

---

## 🚀 Meus Projetos

### 🛒 [E-commerce Backend](https://github.com/gu18r03gi07/ecommerce-backend-4b)

Backend desenvolvido em **Java** para um sistema ecommerce com funcionalidades completas de gerenciamento.

**Stack:** Java • Spring Boot • REST API • MySQL • JPA/Hibernate

### 🎨 [Construção de Layout](https://github.com/gu18r03gi07/Constru-o-de-Layout-atividade)

Projeto de desenvolvimento web focado em criação de **layouts responsivos** com HTML e CSS.

**Stack:** HTML5 • CSS3 • Responsive Design

### 🌐 [Site 01](https://github.com/gu18r03gi07/site01)

Primeiro site completo com **HTML, CSS e JavaScript**, desenvolvido como exercício prático.

**Stack:** HTML5 • CSS3 • JavaScript

### 🐘 [Teste PHP](https://github.com/gu18r03gi07/testephp)

Projeto de teste e aprendizado em **PHP com MySQL**.

**Stack:** PHP • MySQL • HTML/CSS • Docker

---

## 📚 Atualmente Estudando

```
☕ Java & OOP
   ↓
🗄️ Banco de Dados & SQL
   ↓
🔌 APIs REST & Microserviços
   ↓
🌱 Spring Boot & Spring Data
   ↓
🅰️ Angular & TypeScript
   ↓
🌐 Desenvolvimento Full Stack
   ↓
🤖 Inteligência Artificial
   ↓
⚡ Automação & DevOps
   ↓
☁️ Cloud Computing
```

---

## 🎓 Formação & Certificações

**🎓 Engenharia de Software**  
Instituição: UNIFIO Ourinhos  
Status: Em andamento  

**💻 Cursos & Certificações**
- ✅ Fundamentos de Programação
- ✅ Orientação a Objetos com Java
- ✅ Banco de Dados & SQL
- ✅ Desenvolvimento Web (HTML, CSS, JavaScript)
- ✅ REST APIs com Spring Boot
- 📚 Angular (em progresso)
- 📚 Inteligência Artificial (em progresso)

---

## 🎯 Objetivos Profissionais

Meu objetivo é evoluir como desenvolvedor e trabalhar na construção de **sistemas modernos, eficientes e escaláveis**.

### Interesses Principais

* 🚀 Desenvolvimento Full Stack
* ☕ Arquitetura Java Avançada
* 🌱 Spring Boot & Microserviços
* 🅰️ Front-end com Angular
* 🗄️ Otimização de Banco de Dados
* 🔌 Design de APIs REST
* 🤖 Machine Learning & IA
* ⚡ Automação & CI/CD
* ☁️ Cloud (AWS, Azure, GCP)
* 🧩 Arquitetura de Software
* 🚀 Produtos Digitais Inovadores
* 🤝 Open Source Contribution

---

## 📈 Minha Evolução de Aprendizado

```
📚 Fundamentos
      ↓
💻 Programação Básica
      ↓
☕ Java Essencial
      ↓
🧠 Orientação a Objetos
      ↓
🗄️ Banco de Dados & SQL
      ↓
🌐 HTML, CSS & JavaScript
      ↓
🔌 APIs REST & HTTP
      ↓
🌱 Spring Boot Framework
      ↓
🅰️ Angular & TypeScript
      ↓
🌐 Full Stack Integration
      ↓
🤖 IA & Machine Learning
      ↓
💼 Projetos em Produção
```

---

## 📊 GitHub Stats

<div align="center">

[![Gustavo's GitHub stats](https://github-readme-stats.vercel.app/api?username=gu18r03gi07&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true)](https://github.com/gu18r03gi07)

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=gu18r03gi07&layout=compact&langs_count=10&theme=tokyonight&hide_border=true)](https://github.com/gu18r03gi07)

</div>

---

## 💡 O Que Estou Construindo

Atualmente estou buscando transformar meus estudos em projetos cada vez mais completos e desafiadores.

```
💻 Código Limpo
   +
🗄️ Banco de Dados Escalável
   +
🔌 APIs Robustas
   +
🌐 Front-end Responsivo
   +
🤖 Inteligência Artificial
   +
⚡ Automação Inteligente
   +
☁️ Deploy em Cloud
   =
🚀 Soluções Profissionais
```

---

## 🌟 Filosofia de Desenvolvimento

> **"Programar é transformar problemas em soluções. Cada linha de código é uma oportunidade de aprender, crescer e criar impacto."**

Acredito que:
- 🎯 Todo projeto é uma oportunidade de aprendizado
- 💡 Qualidade e limpeza de código são essenciais
- 🤝 Colaboração leva a melhores soluções
- 📚 Aprendizado contínuo é fundamental
- 🚀 Inovação começa com curiosidade

---

## 🌐 Contato & Redes Profissionais

<div align="center">

### 📱 Entre em Contato

[![GitHub](https://img.shields.io/badge/GitHub-gu18r03gi07-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/gu18r03gi07)
[![Email](https://img.shields.io/badge/Email-Contact%20Me-red?style=for-the-badge&logo=gmail&logoColor=white)](mailto:gustavo.rgiamarco@gmail.com)
[![WhatsApp](https://img.shields.io/badge/WhatsApp-Contato-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)](https://wa.me/5514991792178)

</div>

### 📞 Informações de Contato

| Canal | Informação |
|-------|----------|
| **Telefone** | 📞 (14) 99179-2178 |
| **WhatsApp** | 💬 [Enviar mensagem](https://wa.me/5514991792178) |
| **GitHub** | 🐙 [@gu18r03gi07](https://github.com/gu18r03gi07) |
| **Email** | 📧 giamarcogustavo@gmail.com |

### 💼 Disponibilidade

✅ **Aberto para:**
- 🤝 Oportunidades de trabalho e estágio
- 💻 Projetos desafiadores
- 📚 Parcerias e colaboração
- 🚀 Desenvolvimento de produtos
- 🌱 Mentoria e aprendizado contínuo

---

<div align="center">

### 🚀 Obrigado por visitar meu perfil!

⭐ **Explore meus projetos e acompanhe minha evolução como desenvolvedor!**

[⬆ Voltar ao Topo](#-gustavo-rodrigues-giamarco)

</div>
